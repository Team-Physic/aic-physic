...
CLASS_PATTERN = re.compile(r"^(SFP|SC)_(\d+)(\d)$", re.IGNORECASE)
VALID_RAILS = {"SFP": range(5), "SC": range(2)}
VALID_PORTS = {"SFP": range(2), "SC": range(1)}
...
def parse_model_class(class_name: str) -> TargetSpec:
    ...
    port_type, rail_text, port_text = match.groups()
    rail_index, port_index = int(rail_text), int(port_text)
    if rail_index not in VALID_RAILS[port_type]:
        raise ValueError(f"invalid {port_type} rail index: {rail_index}")
    if port_index not in VALID_PORTS[port_type]:
...
def target_from_task(task) -> TargetSpec:
    ...
    if port_type == "SFP":
        ...
        if module is None or port is None:
            raise ValueError(
                "SFP task requires nic_card_mount_<rail>/sfp_port_<port>"
            )
        rail_index, port_index = int(module.group(1)), int(port.group(1))
    elif port_type == "SC":
        module = re.fullmatch(r"sc_port_(\d+)", str(task.target_module_name))
        if module is None or str(task.port_name) != "sc_port_base":
            raise ValueError("SC task requires sc_port_<rail>/sc_port_base")
        rail_index, port_index = int(module.group(1)), 0
    else:
        raise ValueError(f"unsupported task port type: {task.port_type!r}")
...
class PortVision:
    ...
    def load_model(self) -> bool:
        """선택 port type의 YOLO model을 명시 경로에서 lazy-load한다."""
        if self.model is not None:
            return True
        variable = f"AIC_{self.target.port_type}_YOLO_MODEL_PATH"
